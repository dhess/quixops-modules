...
let
  ...
  clientConfig = port: proto: cert: key: tlsAuthKey: ''
  ...
  nodes = {
    client = { config, ... }: {
      ...
      environment.etc."openvpn-bob-vpn1.conf" = {
        text = clientConfig "1194" "udp" bob-cert bob-certKey vpn1-tlsAuthKey;
      };
      environment.etc."openvpn-alice-vpn1.conf" = {
        text = clientConfig "1194" "udp" alice-cert alice-certKey vpn1-tlsAuthKey;
      };
      environment.etc."openvpn-bob-vpn2.conf" = {
        text = clientConfig "443" "tcp-client" bob-cert bob-certKey vpn2-tlsAuthKey;
      };
      environment.etc."openvpn-alice-vpn2.conf" = {
        text = clientConfig "443" "tcp-client" alice-cert alice-certKey vpn2-tlsAuthKey;
      };
    };
