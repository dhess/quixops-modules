# Taken from nixpkgs upstream, adapted to use fetchFromGitHub rather
# than fetchurl because the ffmpeg project does not release tagged
# snapshot tarballs.

# Yes, we use the GitHub mirror, rather than the official ffmpeg git
# repo, because GitHub supports grabbing tarballs rather than the full
# git tree.

{ stdenv, fetchFromGitHub, pkgconfig, perl, texinfo, yasm
...
# Inherit generics
, branch, sha256, version, rev, patches ? [], ...
}:
