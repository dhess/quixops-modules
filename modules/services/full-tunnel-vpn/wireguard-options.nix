...

{
  ...
  privateKeyFile = mkOption {
    ...
    default = null;
    example = literalExample toString ./wg.key;
    description = ''
      A path to a file containing the WireGuard server's private key,
      as generated by the command <command>wg genkey</command>. Note
      that this file will not be copied to the Nix store. However,
      uppon start-up, the service will copy the key to its persistent
      state directory.

      Note: either <option>privateKeyFile</option> or
      <option>privateKeyLiteral</option> must be non-null, but not
      both.
    '';
  };
    ...
    { config, options, name, ... }:
    {
      options = {
        ...
        presharedKeyFile = mkOption {
          type = types.nullOr pkgs.lib.types.nonStorePath;
          default = null;
          example = literalExample toString ./peer.psk;
          description = ''
            A path to a file containing the WireGuard pre-shared key
            for this peer, as generated by the command <command>wg
            genpsk</command>. Note that this file will not be copied
            to the Nix store. However, upon start-up, the service will
            copy the key to its persistent state directory.

            Note: either <option>presharedKeyFile</option> or
            <option>presharedKeyLiteral</option> must be non-null, but
            not both.
         '';
        };
