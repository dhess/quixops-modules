...

{
  ...
  privateKeyFile = mkOption {
    ...
    description = ''
      The WireGuard server's private key, as generated by the command
      <command>wg genkey</command>. Note that this file will not be
      copied to the Nix store. However, uppon start-up, the service
      will copy the key to its persistent state directory.
    '';
  };
    ...
    { config, options, name, ... }:
    {
      options = {
        ...
        presharedKeyFile = mkOption {
          type = pkgs.lib.types.nonStorePath;
          description = ''
            The WireGuard pre-shared key for this client, as generated
            by the command <command>wg genpsk</command>. Note that
            this file will not be copied to the Nix store. However,
